...
import { getAllClubTaskSubmissions, reviewClubTaskSubmission, getAllCloudClubTasks, type CloudClubTaskSubmission, type CloudClubTask } from '@/lib/cloudClubs';

export function CloudClubTaskSubmissionsPanel({ onSubmissionReviewed }: { onSubmissionReviewed?: () => void }) {
  const [submissions, setSubmissions] = useState<CloudClubTaskSubmission[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedSubmission, setSelectedSubmission] = useState<CloudClubTaskSubmission | null>(null);
  ...
  const loadSubmissions = async () => {
    try {
      const data = await getAllClubTaskSubmissions();
      setSubmissions(data);
    } catch (error) {
  ...
  const handleReview = (submission: CloudClubTaskSubmission, action: 'approved' | 'rejected' | 'needs_revision') => {
  ...
  const submitReview = async () => {
    ...
    try {
      await reviewClubTaskSubmission(
