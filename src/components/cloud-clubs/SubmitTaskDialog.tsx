...
import { submitClubTaskForReview, type CloudClubTask } from '@/lib/cloudClubs';
...
export function SubmitTaskDialog({ open, onOpenChange, task, clubId, onSuccess }: SubmitTaskDialogProps) {
  const { user } = useAuth();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (file) {
        ...
        fileUrl = await uploadFileToS3(file, 'cloud-club-files');
        fileName = file.name;
      }

      await submitClubTaskForReview(
        clubId, 
        task.id, 
        comments, 
        user.id, 
